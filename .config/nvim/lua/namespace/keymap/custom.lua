...
vim.keymap.set({ "n", "x" }, "<C-k>", "<C-W><C-k>", { noremap = true, desc = "move to up window" }) -- Careful of conflict with LSP (implemented in LSP section)
...
vim.keymap.set({"n", "x"}, 'k', "v:count == 0 ? 'gk' : 'k'", { expr = true, silent = true, desc = "move up a visual line" })
vim.keymap.set({"n", "x"}, 'j', "v:count == 0 ? 'gj' : 'j'", { expr = true, silent = true, desc = "move down a visual line" })
